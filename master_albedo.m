...
sprintf(['Processing time start: ',datestr(now)])
...
addpath('/Users/andrigun/Documents/GitHub/geo');
data_read_dir = '/Volumes/brunnur/data/MCD43A3';
data_write_dir = '/Volumes/brunnur/data/output';
img_dir = '/Volumes/brunnur/YETI/img';
geo_data_dir = '/Volumes/brunnur/data/geo';
geo = Modis_make_geo_old(geo_data_dir);
% Other settings
draw_pre_post = 0;
...
cd('/Users/andrigun/Dropbox/Brunnur/joklar');
dir_name = ['MCD43A3_alb_cc_',datestr(now,'yyyymmdd')];
mkdir(dir_name);
cd(dir_name);
    plot_albedo;
...
sprintf(['Processing time end: ',datestr(now)])
close all
exit
